...
-- Base workspace number (0, 10, ...) of the currently focused monitor, per
-- vars.monitorWorkspaceBase. Falls back to 0 if the monitor isn't mapped.
local function active_monitor_ws_base()
    local mon = hl.get_active_monitor()
    return (mon and vars.monitorWorkspaceBase[mon.name]) or 0
end
...
local function wsaction(action, range, i)
    return function()
        ...
        if activews then
            ...
            local s  = (i - 1) * 10 + (id % 10)             -- decade i, same slot as current workspace
            local t  = active_monitor_ws_base() + i          -- slot i within the focused monitor's own block
...
-- Focus/move by `delta` (+1/-1), wrapping within the focused monitor's own
-- block of 10 workspaces instead of crossing into the next monitor's block.
local function wsrelative(action, delta)
    return function()
        local base     = active_monitor_ws_base()
        local activews = hl.get_active_workspace()
        local id       = activews and activews.id or (base + 1)
        local offset   = ((id - base - 1 + delta) % 10 + 10) % 10
        local target   = base + offset + 1
